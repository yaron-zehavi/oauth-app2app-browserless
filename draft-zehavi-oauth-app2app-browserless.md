...
However, in {{App2App}} scenarios the web browser must serve as a user-agent, replacing the browser.
...
**native_callback_uri**:
: A new authorization request parameter that holds a deep link that serves as the redirect_uri to the Client App.
...
This is an OAuth authorization endpoint, interoperable with other OAuth RFCs, adapting it from a web-redirecting endpoint to a REST API. It supports the following additional request parameters:
...
In addition, this endpoint

* SHALL NOT use cookies.
* SHALL return Content-Type header with the value "application/json", and a JSON response body.
...
The client app constructs an OAuth authorization request, interoperable with other OAuth RFCs, which also includes the *native_callback_uri* parameter, and sends it to its *Initial Authorization Server*.
...
#### Federating Response
If the authorization server federates the to another AS, it responds with 200 OK and the following JSON response body:

url:
: REQUIRED. A string holding the native_authorization_endpoint of the *Downstream Authorization Server* this AS federates to.

action:
: REQUIRED. A string with the value "call" to indicate that *url* is to be called with HTTP.
...
The client SHALL add the DNS domain of the *url* to to an Allowlist for later invocations as part of the response handling after being invoked by the *User-Interacting Authorization Server' App*.
It then MUST make an HTTP GET request to the returned *url* and process the response as defined in this document.

#### Deep Link Response
If the authorization server authenticates the user, i.e., it does not federate to another *Downstream Authorization Server*, it responds with 200 OK and the following JSON response body:

url:
: REQUIRED. A string holding the deep link claimed by the *User-Interacting App*". Since this URL is intended for the authenticating app, it MAY contain any parameters required for the app and its backend identify and process the authorization request.

action:
: REQUIRED. A string with the value "deep_link" to indicate that *url* is to be invoked natively.

The client app MUST use OS mechanism to invoke the deep link received in *url* and open the *User-Interacting Authorization Server's App*. If no app that claims the deep link can be found, the client app MUST terminate the flow and MAY retry with a non-native flow. See {{fallback}}.

#### Routing Response

If the authorization server needs user input to determine where to federate, it responds with 200 OK and the following JSON body:

id:
: REQUIRED. A string holding the interaction identifier which the AS uses to link the response to the request

action:
: REQUIRED. A string with the value "prompt" to indicate that the client app must prompt the user for input before proceeding.

logo:
: OPTIONAL. URL or base64-encoded logo of Authorization Server. Used for branding 
...
response:
: REQUIRED. A JSON object that holds the URL to which the user input MUST be sent. It only supports to keys, which are mutually exclusive:

* get: The corresponding value is the URL to use for a GET request with user input appended as query params
* post: The corresponding value is the URL to use for a POST request with user input sent in the request body, using application/x-www-form-urlencoded
...
The client app MUST prompt the user according to the response received. It then MUST send the user input to the response endpoint using the requested method including the interaction id.
...
If the authorization server encounters an error, and this error is meant for its client, i.e., the previous/upstream authorization server, it returns 200 OK with the following JSON body:

url:
: REQUIRED. A string holding the redirect_uri of the *Upstream Authorization Server* conveying the OAuth error

action:
: REQUIRED. A string with the value "call" to indicate that *url* is to called with HTTP.
...
The client app MUST make an HTTP GET request to the returned *url* and process the response as defined in this document.

If the authorization server encounters an error, that it cannot/or must not sent to its client, it responds with 4xx/5xx and the following JSON body:
...
The client app SHOULD display an appropriate error message to the user and terminate the flow.
In case of *native_app2app_unsupported*, the client MUST terminate the flow and MAY retry with a non-native flow. See {{fallback}}.
...
If the the app is invoked with an *error* (and optional *error_description*) parmeter, or no parameter at all, it MUST terminate the flow and SHOULD display an appropriate message to the user.
